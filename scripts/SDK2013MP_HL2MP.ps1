...
{
  if (-not (Test-SteamAppInstalled 243750))
  {
    throw "Source SDK Base 2013 Multiplayer not installed!"
  }

  if (-not (Test-DirectoryExists -Path $SDKBASE2013MP))
  {
    throw "Source SDK Base 2013 Multiplayer path not found!"
  }
}
...
if ($All.IsPresent -or $Base.IsPresent) {
  ...
  . "$PSScriptRoot\private\install_base_content.ps1" `
    ...
    -OutputVPKBaseName "sdkbase2013mp_hl2mp_base_converted" `
    -VPKRootFolder hl2mp `
    -Uninstall:$Uninstall
}
